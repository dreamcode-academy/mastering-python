...
df['Temperature'] = df['Detailed Forecast'].apply(extract_temprature)

df['Wind Speed'] = df['Detailed Forecast'].apply(extract_wind_speed)

df['Time of Day'] = df['Period'].apply(lambda x: 'Night' if 'Night' in x else 'Day')
